...
ORDER BY 2 ASC; --System 계정으로 실행 GRANT 뒤에 사용할 수 있는 녀석들이 나온다

-- * DCL(Data Control Language) : GRANT / REVOKE
-- 권한부여 및 회수는 관리자 세션에서 사용가능
-- 관리자 계정
-- 1. system
-- 2. sys
...
SELECT * FROM ROLE_SYS_PRIVS WHERE ROLE = 'CONNECT'; -- KH 계정에서 사용
--ROLE에 부여된 테이블 권한
...
-- - 단점 : 인덱스를 위한 추가 저장공간이 필요하고 인덱스를 생성하는데 시간이 걸림
