...
FROM EMPLOYEE
JOIN DEPARTMENT
ON DEPT_CODE = DEPT_ID WHERE DEPT_CODE IN ('D5','D6','D9')
ORDER BY 2 ASC;
...
-- 데이터 없음!
--SAL_GRADE - MAX_SAL
SELECT EMP_NAME, DEPT_TITLE, SALARY "급여", SALARY*12 "연봉"
FROM EMPLOYEE
...
--8. 보너스포인트가 없는 직원들 중에서 직급이 차장과 사원인 직원들의 사원명,
-- 직급명, 급여를 조회하시오. 
--단, join과 IN 사용할 것
...
--9. 재직중인 직원과 퇴사한 직원의 수를 조회하시오.(JOIN아님) -- GROUP
SELECT DECODE(ENT_YN,'N','재직','퇴직'), COUNT(*)
FROM EMPLOYEE
GROUP BY ENT_YN;
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
INNER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 21개 ,2개가 빠짐
-- DEPT_CODE NULL값인 데이터는 빠짐
-- → 이것을 INNER JOIN이라고 함

SELECT COUNT(*) FROM EMPLOYEE; -- 23

--LEFT OUTER JOIN은 왼쪽테이블이 가지고있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 23행

--RIGHT OUTER JOIN은 오른쪽테이블이 가지고있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
RIGHT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 24행
