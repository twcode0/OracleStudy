...
/*
    ...
    (실질적인 데이터를 저장하고있지 않지만 하나의 테이블처럼 사용 가능함)
...
-- 1.2.1 Stored View(저장 O) - 이름을 붙여서 저장함.
    ...
    FROM EMPLOYEE;
-- 주의 : VIEW를 만들때는 권한이 필요함, 권한명 : CREATE VIEW
-- System 계정으로 권한 부여
...
--View V_EMPLOYEE(가) 삭제되었습니다.
CREATE VIEW V_EMPLOYEE 
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE;
SELECT * FROM V_EMPLOYEE;
--1.5.1 수정하기 OR REPLACE
...
-- VIEW 에 대해 조회만 가능하며 DML 불가능하게 하는 옵션
...
--USER_NO 컬럼에 저장되는 데이터는 1부터 시작하여 1씩증가함.
-- 일용자일때는1, 이용자일때는 2,... 
-- USER_NO 컬럼에 들어가는 데이터는 누군가 기억하고 있어야함.
-- 마지막 번호가 몇번인지 몇씩 증가해서 들어가는지를 기억해야함.
-- 그런 역할을 하는 것이 바로 Sequence이다.

-- 2.1 개요
-- - 순차적으로 정수 값을 자동으로 생성하는 객체, 자동 번호 발생기(채번기)의
-- 역할을 함.
-- - 사용법 : CREATE SEQUENCT [시퀀스명]; → 기본값으로 생성 (1부터 1씩증가)
-- 2.2 Squence 옵션
-- 2.2.1 MINVALUE : 발생시킬 최소값 지정
-- 2.2.2 MAXVALUE : 발생시킬 최대값 지정
-- 2.2.3 START WITH : 처음 발생시킬 시작값 지정, 기본값1
-- 2.2.4 INCERMENT BY : 다음 값에 대한 증가치, 기본값 1
-- 2.2.5 NOCYCLE : 시퀀스값이 최대값까지 증가를 완료하면 CYCLE은 START WITH로 다시 시작
-- 2.2.6 NOCACHE : 메모리상에서 시퀀스값을 관리, 기본값 20, NOCACHE를 안하면 갑자기 시퀀스값이 증가핟게 됨
...
-- SELECT의 뒤 도는 INSERT INTO VALUSE의 전달값으로 작성
SELECT SEQ_KH_USER_NO.NEXTVAL FROM DUAL; -- 첫 NEXTVAL은 시퀀스를 개시함. 필수조건
SELECT SEQ_KH_USER_NO.CURRVAL FROM DUAL; -- NEXTVAL을 실행한 후 현재 시퀀스 값 조회
...
SELECT SEQ_ORDER_NO.NEXTVAL FROM ORDER_TBL;
...
CREATE SEQUENCE SEQ_KH_MEM_JOIN
START WITH 1
INCREMENT BY 1
NOCYCLE
NOCACHE;
...
VALUES (SEQ_KH_MEM_ID.NEXTVAL, '홍길동', '20', SEQ_KH_MEM_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES (SEQ_KH_MEM_ID.NEXTVAL, '청길동', '30', SEQ_KH_MEM_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES (SEQ_KH_MEM_ID.NEXTVAL, '외길동', '40', SEQ_KH_MEM_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES (SEQ_KH_MEM_ID.NEXTVAL, '고길동', '50', SEQ_KH_MEM_JOIN.NEXTVAL);
