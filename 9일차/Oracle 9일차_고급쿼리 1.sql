...
-- 1. TOP-N 분석
-- 특정 컬럼에서 가장 큰 N개의 값 도는 가장 작은 N개의 값을 구해야 할 경우에 사용
-- 예시) 가장 적게 팔린 제품 10가지? 회사에서 가장 소득이 높은 사람 3명은?
...
-- 뒤죽박죽 나오는 이유 → 순서가 매겨진 후 정렬하게 되니까
...
SELECT ROWNUM, E.* FROM
(SELECT * FROM EMPLOYEE WHERE DEPT_CODE = 'D5' AND SALARY IS NOT NULL ORDER BY 1 DESC) E
WHERE ROWNUM <4;

-- @실습문제2
-- 부서별 급여평균 TOP3 부서의 부서코드와 부서명, 평균급여를 출력하세요.
--(SELECT DEPT_TITDE FROM DEPARTMENT WHERE DEPT_CODE = DEPT_ID)??
SELECT * FROM
...
-- 2. WITH
-- 서브쿼리에 이름을 붙여주고 인라인뷰로 사용시 서브쿼리의이름을 FROM절에 기술할 수 있음
...
-- D5부서에서 연봉 TOP3의 전체정보를 출력하세요.
WITH TOP3_D5 AS
(SELECT * FROM EMPLOYEE WHERE DEPT_CODE = 'D5' AND SALARY IS NOT NULL
ORDER BY SALARY DESC)
SELECT * FROM TOP3_D5
WHERE ROWNUM <=3;
...
-- @실습문제2
-- 부서별 급여평균 TOP3 부서의 부서코드와 부서명, 평균급여를 출력하세요.
WITH TOP_DEPT_SAL AS
(SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY))
FROM EMPLOYEE
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
GROUP BY DEPT_CODE, DEPT_TITLE
ORDER BY 3 DESC)
SELECT * FROM TOP_DEPT_SAL
WHERE ROWNUM <= 3;
...
-- PRIOR 다음에 나오는 컬럼은 WITH에서 사용된 컬럼
...
-- 특정 컬럼 기준으로 랭킹ㅇ을 부여함, 중복순위 다운은 해당 갯수만큼 건너뒤고 반환함.
-- 예제) 회사의 연봉 순위를 출력하시오
...
SELECT
    RANK() OVER(ORDER BY HIRE_DATE DESC) "HIRE_RANK"
    ,EMP_NAME
    ,HIRE_DATE
...
-- 4.2 DENSE_RANK() OVER
-- → 중보순위 상관없이 순차적으로 반환, 빠짐없이 빽빽한 순위를 부여함.
...
WHERE ROWNUM <11; -- ROWNUM이랑 RANK랑 똑같다는 가정하에 사용 가능
