...
--      d. LTRIM/RTRIM     : 특정문자 제거(공백제거)
        ...
        g. CONAT           : 문자열 합쳐줌
        h. REPLACE         : 문자열 바꿔줌
        사용 방법 : 함수명(값1, 값2, ...) , 값에는 리터럴이나 컬럼명을 넣어주면 됨
...
-- b. DECODE 함수
-- 사용법 : DECODE( 값, 조건값, 참일때 값, 조건값, 참일때 값, 모두 아닐때 값)
--          DECODE( 변하는 값, 1, '빨강', 2, '노랑', '파랑')
-- c. CASE 함수
...
SELECT NVL(DEPT_CODE,0), SUM(SALARY), FLOOR(AVG(SALARY)), COUNT(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1 ASC;

-- 그룹실습 2
-- EMPLOYEE 테이블에서 부서코두, 보너스를 지급받는 사원 수를 조회하고 부서코드 순으로 정렬하세요
...
-- 그룹실습3
-- EMPLOYEE 테이블에서 직급이 J1인 사람들을 제외하고 직급별 사원수 및 평균 급여를 출력하세요.
SELECT JOB_CODE, FLOOR(AVG(SALARY)) FROM EMPLOYEE
WHERE JOB_CODE NOT LIKE 'J1'
-- WHERE JOB_CODE != 'J1'
-- HWERE JOB_CODE NOT IN 'J1'
-- WHERE JOB_CODE <> 'J1'
GROUP BY JOB_CODE;
...
ORDER BY 1 ASC; -- 1 = TO_CHAR(HIRE_DATE,'YYYY')

-- 그룹실습5
-- EMPLOYEE 테이블에서 EMP_NO의 8번째 자리가 1, 3이면 '남', 2, 4이면 '여'로 결과를 조회하고,
-- 성별별 급여의 평균(정수처리), 급여의 합계, 인원수를 조회한 뒤 인원수로 내림차순을 정렬하시오.
-- DECODE(IF) OR CASE(SWITCH) 사용
SELECT DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무') "성별", FLOOR(AVG(SALARY)), SUM(SALARY), COUNT(SALARY)
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무')
;

-- 지금까지 한 것
-- 숫자 처리함수, 문자 처리 함수, 날짜 처리 함수
...
-- 부서내 성별 인원수를 구하시오
SELECT 
DEPT_CODE, DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무') "성별",
COUNT(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- SELECT 문에서 조건을 걸때는 WHERE 를 사용하지만
-- GROUP BY 한 결과값에 조건을 걸 때에는 HAVING을 사용한다.
SELECT * FROM EMPLOYEE
...
SELECT DEPT_CODE, COUNT(DEPT_CODE) "부서별인원수" FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(*) < 5
ORDER BY 1 ASC;
...
SELECT DEPT_CODE, COUNT(DEPT_CODE) AS 인원수 FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(*) > 5;

-- HAVING 실습문제2
--부서내 직급별 인원수가 3명이상인 직급의 부서코드, 직급코드, 인원수를 출력하세요.
SELECT  DEPT_CODE, JOB_CODE, COUNT(JOB_CODE) FROM EMPLOYEE
GROUP BY JOB_CODE, DEPT_CODE 
HAVING COUNT(*) >= 3;

-- HAVING 실습문제3
-- 매니저가 관리하는 사원이 2명 이상인 매니져아이디와 관리하는 사원수를출력하세요.
...
SELECT SYSDATE "입대날짜", ADD_MONTHS(SYSDATE, 18) "제대 일자",
...
--	ex) 전지연		1997년 3월
DESC EMPLOYEE;
SELECT EMP_NAME "직원명", EXTRACT(YEAR FROM HIRE_DATE)||'년 '
...
--	* 주말도 포함 , 소수점 아래는 버림
SELECT EMP_NAME "이름", HIRE_DATE "입사일", FLOOR(SYSDATE-HIRE_DATE) "근무일수"
FROM EMPLOYEE;
...
--10. 직원명, 직급코드, 연봉(원) 조회
--  단, 연봉은 ￦57,000,000 으로 표시되게 함 -- 우측부터 3개씩 반복해서 잘라서 , 넣어야함
--     연봉은 보너스포인트가 적용된 1년치 급여임
SELECT EMP_NAME"직원명", JOB_CODE "직급코드", '\'||SALARY||'원' "연봉(원)" 
FROM EMPLOYEE;
--11. 사원명과, 부서명을 출력하세요.
